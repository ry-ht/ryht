...
impl LeadAgent {
    ...
    async fn execute_workers_parallel(
        ...
        for batch in handles_with_tasks.chunks(batch_size) {
            ...
            // Process results and release workers
            for (result, (handle, _delegation)) in batch_results.into_iter().zip(batch.iter()) {
                ...
                match result {
                    ...
                    Err(e) => {
                        error!("Worker {} execution failed: {}", handle.worker_id, e);
                        // Continue with other workers
                    }
                }

                // Release worker back to registry
                let mut registry = self.worker_registry.write().await;
                if let Err(e) = registry.release_worker(&handle.worker_id, success) {
                    warn!("Failed to release worker {}: {}", handle.worker_id, e);
                }
            }
        }
    ...
    async fn send_task_to_worker(
        ...
        use cortex_coordination::{MessageEnvelope, Message};
