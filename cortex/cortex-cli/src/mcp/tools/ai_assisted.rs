...
impl Tool for AiGenerateDocstringTool {
    ...
    async fn execute(&self, input: Value, _context: &ToolContext) -> std::result::Result<ToolResult, ToolError> {
        ...
        // Generate comprehensive documentation using code analysis
        let output = self.generate_documentation(&input).await
            .map_err(|e| ToolError::ExecutionFailed(e.to_string()))?;
...
impl AiGenerateDocstringTool {
    ...
    async fn generate_documentation(&self, input: &AiGenerateDocstringInput)
        -> std::result::Result<AiGenerateDocstringOutput, String> {
        ...
        let signature = unit.get("signature").and_then(|v| v.as_str()).unwrap_or("");
        let body = unit.get("body").and_then(|v| v.as_str()).unwrap_or("");
        let return_type = unit.get("return_type").and_then(|v| v.as_str());

        // Generate documentation based on style
        let docstring = match input.style.as_str() {
            "rustdoc" | "rust" => self.generate_rustdoc(name, signature, body, return_type, input.include_examples),
            _ => self.generate_rustdoc(name, signature, body, return_type, input.include_examples),
        };

        // Calculate quality score
        let quality_score = self.calculate_doc_quality(&docstring);

        Ok(AiGenerateDocstringOutput {
            docstring,
    ...
    fn generate_rustdoc(&self, name: &str, signature: &str, body: &str, return_type: Option<&str>, include_examples: bool) -> String {
        let mut doc = String::new();

        // Main description
        doc.push_str(&format!("/// {}\n", self.generate_description(name, body)));
        doc.push_str("///\n");

        // Parse parameters from signature
        if let Some(params) = self.extract_parameters(signature) {
            if !params.is_empty() {
        ...
        if let Some(ret_type) = return_type {
            if ret_type != "()" && !ret_type.is_empty() {
                ...
                doc.push_str(&format!("/// Returns a `{}` value\n", ret_type));
                doc.push_str("///\n");
            }
        }

        // Error conditions
        if body.contains("Result") || body.contains("?") {
            doc.push_str("/// # Errors\n");
        ...
        // Panics
        if body.contains("unwrap()") || body.contains("expect(") || body.contains("panic!") {
            doc.push_str("/// # Panics\n");
            doc.push_str("/// May panic if internal assertions fail\n");
            doc.push_str("///\n");
        }
        ...
        if include_examples {
            ...
            doc.push_str("///\n");
            doc.push_str("/// ```\n");
            doc.push_str(&format!("/// let result = {}(/* params */);\n", name));
            doc.push_str("/// ```\n");
        }
    ...
    fn generate_description(&self, name: &str, body: &str) -> String {
        // Convert snake_case to words
        let words: Vec<&str> = name.split('_').collect();
        let action = words.first().unwrap_or(&"Performs");

        // Analyze what the function does
        if body.contains("return") {
            format!("{} operations and returns a computed value", action.to_uppercase())
        } else if body.contains("async") {
            format!("{} asynchronous operations", action.to_uppercase())
        } else if body.contains("println!") || body.contains("eprintln!") {
            format!("{} operations and outputs results", action.to_uppercase())
        } else {
            format!("{} the specified operation", action.to_uppercase())
        }
    }

    fn extract_parameters(&self, signature: &str) -> Option<Vec<(String, String)>> {
        // Simple parameter extraction from signature
        if let Some(params_str) = signature.split('(').nth(1)?.split(')').next() {
    ...
    fn calculate_doc_quality(&self, doc: &str) -> f32 {
        let mut score: f32 = 0.0;

        if doc.contains("# Parameters") { score += 0.2; }
        if doc.contains("# Returns") { score += 0.2; }
        if doc.contains("# Examples") { score += 0.3; }
        if doc.contains("# Errors") { score += 0.15; }
        if doc.contains("# Panics") { score += 0.15; }
        if doc.lines().count() > 5 { score += 0.1; }
