//! Test dependency analysis on real codebase (cortex-code-analysis)

use cortex_mcp::graph_algorithms::*;

/// Build a graph from cortex-code-analysis actual dependencies
fn build_cortex_code_analysis_graph() -> Graph {
    let mut graph = Graph::new();

    // Real dependencies from cortex-code-analysis/src/lib.rs
    graph.add_edge("cortex_code_analysis".to_string(), "dependency_extractor".to_string());
    graph.add_edge("cortex_code_analysis".to_string(), "types".to_string());
...
#[test]
fn test_cortex_code_analysis_has_no_cycles() {
    let graph = build_cortex_code_analysis_graph();

    let cycles = find_cycles(&graph);

    // Good code should have no cycles
    assert_eq!(cycles.len(), 0, "cortex-code-analysis should have no circular dependencies");
}

#[test]
fn test_cortex_code_analysis_layers() {
    let graph = build_cortex_code_analysis_graph();
...
#[test]
fn test_cortex_code_analysis_entry_points() {
    let graph = build_cortex_code_analysis_graph();

    let roots = find_roots(&graph);

    // Main module should be a root
    assert!(roots.contains(&"cortex_code_analysis".to_string()), "Main module should be an entry point");
}

#[test]
fn test_cortex_code_analysis_utilities() {
    let graph = build_cortex_code_analysis_graph();
...
#[test]
fn test_cortex_code_analysis_hub_detection() {
    let graph = build_cortex_code_analysis_graph();
...
#[test]
fn test_dependency_path_cortex_code_analysis() {
    let graph = build_cortex_code_analysis_graph();
...
fn test_impact_of_changing_types() {
    ...
    // Should impact dependency_extractor and cortex_code_analysis
