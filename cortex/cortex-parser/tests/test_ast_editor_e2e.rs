...
use cortex_parser::{AstEditor, Range};
...
"#
    ...
    // Extract lines 2-4 into a new function
    let result = editor
        .extract_function(2, 4, "calculate_sum")
        .unwrap();

    assert_eq!(result.function_name, "calculate_sum");

    editor.apply_edits().unwrap();
    let code = editor.get_source();

    // Should have the new function and a call to it
